...
function run_vecmat_benchmarks(n::Int, p::Int; tmp=nothing, FlagType=UInt8)
    ...
    KernelForge_stats = bench("KernelForge.vecmat!", () -> KernelForge.vecmat(*, +, x, A; FlagType=FlagType, tmp=tmp))
...
function run_all_benchmarks(sizes::Vector{Int}; tmp=nothing, FlagType=UInt8)
    ...
    for total in sizes
        # Generate n values that divide total evenly
        n_values = filter(n -> total % n == 0, [10, 100, 1000, 10_000, 100_000, 1_000_000])
        n_values = filter(n -> n <= total, n_values)

        for n in n_values
            p = total ÷ n
            stats = run_vecmat_benchmarks(n, p; tmp=tmp, FlagType=FlagType)
...
If `df2` is provided, an additional horizontal line is shown above the KernelForge kernel time
...
function plot_kernel_comparison(
    ...
    for n in n_values
        KernelForge_row = filter(r -> r.n == n && r.method == "Forge", subset)
        cublas_row = filter(r -> r.n == n && r.method == "cuBLAS", subset)
    ...
    if df2 !== nothing
        ...
        for n in n_values
            KernelForge_row2 = filter(r -> r.n == n && r.method == "Forge", subset2)
...
"""
    save_plots(figures::Dict{Int, Figure}, prefix::String="vecmat_benchmark")

Save all figures to PNG files.
"""
function save_plots(figures::Dict{Int,Figure}, prefix::String="vecmat_benchmark")
...
run_vecmat_benchmarks(1000000, 1; tmp=tmp)
