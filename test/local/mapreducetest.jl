...
#%%
n = 1000000
src = cu(Int.([i for i in (1:n)]))
dst = cu([Int(0)])
tmp = get_allocation(Luma.mapreduce1d!, f, op, dst, (src,); FlagType=UInt64, config=(workgroup=256, blocks=400))
...
@testset "mapreduce correctness" begin
    ns = reverse([33, 100, 1_000, 10_001, 100_000, 1_000_000])
    ...
    for n in ns
        @testset "n=$n" begin
            for trial in 1:10
                ...
                @test isapprox(luma_result.x, base_result.x, rtol=1e-4, atol=1e-5)
                @test isapprox(luma_result.y, base_result.y, rtol=1e-4, atol=1e-5)
                @test isapprox(luma_result.z, base_result.z, rtol=1e-4, atol=1e-5)
            end
        end
    end
end